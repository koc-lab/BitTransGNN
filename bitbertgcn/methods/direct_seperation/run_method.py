...
from utils import get_quant_name, load_bitbertgcn_for_inference, get_model_type, get_train_state, set_bitbert_ckpt_dir
from data.loader.dataloaders import TextDataObject
from models import BertClassifier
from inference_engines import BitBERTInference

def run_bitbertgcn_for_direct_seperation(config):
        ...
        loaded_bitbertgcn_joint_training = parameters["joint_training"]
        loaded_bitbertgcn_quantize_gcn = model_configs["quantize_gcn"]
        checkpoint_dir = exp_configs["checkpoint_dir"]
        inference_type = exp_configs["inference_type"]
        
        train_state = get_train_state(loaded_bitbertgcn_joint_training)
        model_type = get_model_type(quantize_bert, loaded_bitbertgcn_quantize_gcn)
        ...
        ckpt_dir_dict = set_bitbert_ckpt_dir(checkpoint_dir, quantize_bert, quantize_embeddings, bert_pre_model, quant_name, dataset_name, num_bits_act, inference=True, direct_seperation=True, bitbertgcn_inference_type=inference_type)
        ...
        model = quantize_bitbert_for_inference(model, ckpt, loaded_bitbertgcn_joint_training, quantize_bert, num_states, loaded_bitbert_quant_type, quantize_embeddings, num_bits_act)
