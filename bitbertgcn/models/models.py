...
"""
The models included below are written in other libraries to allow model's use within different coding frameworks
They are not used within the experiments of this project.
"""

class BertGCNTorchGeometric(nn.Module):
    """
    BERTGCN model coded in Torch Geometric library in case one wants to replicate the code using the Torch Geometric framework
    This model is not used within this project
    """
    def __init__(self, pretrained_model='roberta-base', joint_training=False, binarize_gcn=False, nb_class=20, m=0.7, gcn_layers=2, n_hidden=200, dropout=0.5):
        super(BertGCNTorchGeometric, self).__init__()
        self.m = m
        self.joint_training = joint_training
        self.nb_class = nb_class
        self.tokenizer = AutoTokenizer.from_pretrained(pretrained_model)
        ...
        self.feat_dim = list(self.bert_model.modules())[-2].out_features
        self.classifier = nn.Linear(self.feat_dim, nb_class)
        self.gcn = torch_geometric.nn.Sequential('x, edge_index, edge_weight', [
            (GCNConv(in_channels=self.feat_dim, out_channels=n_hidden), 'x, edge_index -> x'),
            nn.ReLU(inplace=True),
            nn.Dropout(inplace=True),
            (GCNConv(in_channels=n_hidden, out_channels=nb_class), 'x, edge_index -> x')
        ])

    def forward(self, graph_data, idx):
        if self.joint_training:
            input_ids, attention_mask = graph_data['input_ids'][idx], graph_data['attention_mask'][idx]
            if self.training:
                cls_feats = self.bert_model(input_ids, attention_mask)[0][:, 0]
        ...
        gcn_logit = self.gcn(x=graph_data["cls_feats"], edge_index=graph_data["edge_index"], edge_weight=graph_data["edge_weight"])[idx]
