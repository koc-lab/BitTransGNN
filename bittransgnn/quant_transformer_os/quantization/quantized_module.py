...
from .fake_quant import GroupFixedFakeQuantize, FixedFakeQuantize, LSQFakeQuantize, LSQPlusFakeQuantize
...
def ActivationQuantizer(a_qconfig):
    if FakeQuantizeDict[a_qconfig.quantizer] is GroupFixedFakeQuantize:
        return FakeQuantizeDict[a_qconfig.quantizer](
                ...
                group_size=getattr(a_qconfig, 'group_size', 128))
    else:
        return FakeQuantizeDict[a_qconfig.quantizer](
                ObserverDict[a_qconfig.observer],
                bit=a_qconfig.bit,
...
def WeightQuantizer(w_qconfig):
    if FakeQuantizeDict[w_qconfig.quantizer] is GroupFixedFakeQuantize:
        return FakeQuantizeDict[w_qconfig.quantizer](
                ...
                group_size=getattr(w_qconfig, 'group_size', 128))
    else:
        return FakeQuantizeDict[w_qconfig.quantizer](
                ObserverDict[w_qconfig.observer],
                bit=w_qconfig.bit,
