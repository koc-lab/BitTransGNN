...
import logging
from quant_transformer_os.quantization import QuantizedModule
...
class QuantizedBertModel(QuantizedModule, ModuleUtilsMixin):
    ...
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic'):
        ...
        if org_module.pooler is None or isinstance(org_module.pooler, nn.Identity):
...
class QuantizedBertClassifier(QuantizedModule):
    def __init__(self, org_module, w_qconfig, a_qconfig, qoutput=True, backend='academic', is_remove_padding=False):
        ...
        #self.dropout = org_module.bert.dropout
    ...
    def forward(self, features, **kwargs):
        ...
        x = self.classifier(x)
        if self.qoutput:
            x = self.classifier_post_act_fake_quantize(x)
        return x
    ...
    def forward(
        ...
        # Take [CLS] token from last_hidden_state
...
class QuantizedBertForSeqClsNoPooler(QuantizedModule):
    """
    Quantized version of your original BertClassifier:
      logits = Linear(last_hidden_state[:, 0])
    No pooler, no added dropout. Only quantization hooks inserted.
    """
    def __init__(self, org_module, w_qconfig, a_qconfig,
                 qoutput=True, backend='academic',
                 is_remove_padding=False, hf_output=True):
        """
        org_module: your *fp32* model whose weights we copy from.
                    Accepts either your BertClassifier or a BertForSeqClsNoPooler
                    (must expose .bert_model or .bert, and .classifier).
        w_qconfig, a_qconfig: your quantization configs
        qoutput:  if True, apply post-act fake-quant to logits
        hf_output: if True, return SequenceClassifierOutput; else return {"cls_logit": logits} like your original
        """
        super().__init__(backend)
        self.is_remove_padding = is_remove_padding
        self.qoutput = qoutput
        self.hf_output = hf_output

        # Determine where the encoder and classifier live on the fp32 module
        enc_attr = 'bert_model' if hasattr(org_module, 'bert_model') else 'bert'
        self.num_labels = getattr(org_module, 'num_labels', getattr(org_module, 'classifier', None).out_features)
        self.config = getattr(org_module, 'config', None)  # may be None if you used a plain nn.Module

        # 1) Quantized encoder (copy weights from fp32 encoder)
        self.bert = QuantizedBertModel(
            org_module=getattr(org_module, enc_attr),
            w_qconfig=w_qconfig,
            a_qconfig=a_qconfig,
            qoutput=False,           # encoder produces fp activations into head (same as your other quant modules)
            backend=self.backend
        )

        if isinstance(org_module.pooler, nn.Identity) or org_module.pooler is None:
            self.pooler = None
        else:
            self.pooler = QuantizedBertPooler(org_module.pooler, w_qconfig, a_qconfig,
                                            qoutput=self.qoutput, backend=self.backend)
        
        # 2) Post-CLS activation quant (optional but typical)
        self.cls_post_act_fake_quantize = Quantizer(None, a_qconfig)

        # 3) Classifier: wrap Linear with weight quant
        #    If org_module.classifier is a Linear, Quantizer(module, w_qconfig) will quantize its weights
        self.classifier = Quantizer(org_module.classifier, w_qconfig)

        # 4) Optional post-logits quant (to quantize outputs, if desired)
        if self.qoutput:
    ...
    def forward(
        ...
        # Preserve HF semantics when config is available
        if self.config is not None:
            return_dict = self.config.use_return_dict if return_dict is None else return_dict
        else:
            return_dict = True if return_dict is None else return_dict

        # Optional padding removal mask your encoder expects
        observation_mask = attention_mask.sum(1) if (self.is_remove_padding and attention_mask is not None) else None
        ...
        enc_out = self.bert(
            ...
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=True,                 # enforce dict for clarity
            observation_mask=observation_mask
        )
        last_hidden_state = enc_out.last_hidden_state  # (B, T, H)

        pooler_output = None
        if self.pooler is not None:
            pooler_output = self.pooler(last_hidden_state)
            
        # ---- Head: exactly your original math ----
        cls_feat = last_hidden_state[:, 0]            # CLS
        q_cls_feat = self.cls_post_act_fake_quantize(cls_feat)
        logits = self.classifier(q_cls_feat)          # Linear (weight-quantized)
        if self.qoutput:
            logits = self.logits_post_act_fake_quantize(logits)

        # ---- Loss (only when using HF Trainer or you pass labels) ----
        loss = None
        if labels is not None:
            # Match HF logic so it "just works" with Trainer
            num_labels = getattr(self, 'num_labels', logits.shape[-1])
            # In case self.config is None, infer problem type like HF
            problem_type = None
            if self.config is not None and getattr(self.config, "problem_type", None):
        ...
        # ---- Outputs ----
        if not return_dict:
            # tuple-style (HF compatibility)
            out = (logits, enc_out.hidden_states, enc_out.attentions)
        ...
        if self.hf_output:
            ...
            # For your original API
            # NOTE: original returned {"cls_logit": cls_logit.clone()}
            # cloning is unnecessary here, but keep semantics if you want:
            return {"cls_logit": logits.clone()}
