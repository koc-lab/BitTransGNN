import copy
from quant_transformer_os.model.quant_bart import QuantizedBartForConditionalGeneration, \
    QuantizedBartForSequenceClassification, QuantizedBartForQuestionAnswering  # noqa: F401
from quant_transformer_os.model.quant_bert import QuantizedBertForSequenceClassification, \
    QuantizedBertForQuestionAnswering, QuantizedBertClassifier, QuantizedBertForSeqClsNoPooler  # noqa: F401
from quant_transformer_os.model.quant_roberta import QuantizedRobertaForSequenceClassification, \
...
def get_model_task_type(model_name, config_data):
    if config_data.dataset_name in ('cola', 'mnli', 'mrpc', 'qnli', 'qqp', 'sst2', 'rte', 'stsb'):
        task_type = 'glue'
    elif config_data.dataset_name in ("20ng", "mr", "ohsumed", "R8", "R52"):
...
def quantize_model(fp_model, config):
    ...
    print("Quantized" + str(fp_model.__class__.__name__))
