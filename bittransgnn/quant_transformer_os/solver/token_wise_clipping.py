...
import torch
import logging
from quant_transformer_os.quantization.fake_quant import QuantizeBase, LSQPlusFakeQuantize, LSQFakeQuantize
from quant_transformer_os.quantization.state import disable_all
    ...
    with torch.no_grad():
        for i, batch in enumerate(fp_input):
            ...
            if fp_output is not None:
                #if task_type == 'glue':
                if task_type in ('glue', 'text-classification'):
