from quantization.binarize_model import quantize_bittransformer_for_inference
...
def run_bittransgnn_for_direct_separation(config):
        ...
        ckpt_dir_dict = set_bittrans_ckpt_dir(checkpoint_dir, quantize_bert, quantize_embeddings, quantize_attention, bert_pre_model, quant_name, dataset_name, num_bits_act, adj_type=adj_type, inference=True, direct_induction=True)

        regression = dataset_name == "stsb"

        model = BitTransformer(pretrained_model=bert_pre_model, nb_class=nb_class, regression=regression)
        model = quantize_bittransformer_for_inference(model, ckpt, loaded_bittransgnn_joint_training, quantize_bert, num_states, loaded_bitbert_quant_type, quantize_embeddings, num_bits_act, quantize_attention=quantize_attention, linear_backend=linear_backend, embedding_backend=embedding_backend, attn_backend=attn_backend)
