from bittransgnn_ops_calc import bert_ops, gcn_ops
...
def bert_energy_info(model_type, model_size, exact=True, inv=False, dataset_conf: Optional[dict] = None, full_batch=False):
    highbit = 32.0
...
def bertgcn_energy_info(model_type, model_size, gcn_conf: dict, exact=True, inv=False, dataset_conf: Optional[dict] = None, full_batch=True, batch_size=32, train_type="static", gcn_bits=32):
    highbit = 32.0
    bit_list = [32.0, 1.0, 2.0]
    processor_list = ["7nm", "45nm"]
    for processor in processor_list:
        for bits in bit_list:
            ...
            for dtype in dtype_list:
                ...
                gcn_add_ops, gcn_mult_ops = gcn_ops(num_nodes, num_edges, dh, dg, do, bits=gcn_bits, dataset_conf=dataset_conf, train_type=train_type, batch_size=batch_size)
                ...
                #    gcn_add_energy, gcn_mult_energy = gcn_add_energy * num_sequences / batch_size, gcn_mult_energy * num_sequences / batch_size
                #    gcn_energy = gcn_energy * num_sequences / batch_size
                total_energy += gcn_energy
                add_energy += gcn_add_energy
                mult_energy += gcn_mult_energy
                #if bits == highbit:
                #    print("add energy: ", add_energy)
                #    print("mult energy: ", mult_energy)
                #    print("total energy: ", total_energy)
...
def get_bert_total_energy(model_type, model_size, bits, dtype, dataset_conf: Optional[dict] = None, processor = "7nm", exact=True, inv=False, add_gcn=False, gcn_conf: Optional[dict] = None, full_batch=False, train_type="static", batch_size=32):
    if add_gcn:
    ...
    if add_gcn:
        num_nodes, num_edges, dh, dg, do, gcn_bits = gcn_conf["num_nodes"], gcn_conf["num_edges"], gcn_conf["dh"], gcn_conf["dg"], gcn_conf["do"], gcn_conf["gcn_bits"]
        #gcn_add_ops, gcn_mult_ops = gcn_ops(num_nodes, num_edges, dh, dg, do)
        #gcn_add_ops, gcn_mult_ops = gcn_quant_ops(num_nodes, num_edges, dh, dg, do)
        gcn_add_ops, gcn_mult_ops = gcn_ops(num_nodes, num_edges, dh, dg, do, bits=gcn_bits, dataset_conf=dataset_conf, train_type=train_type, batch_size=batch_size)
        gcn_add_energy, gcn_mult_energy = tot_energy(gcn_add_ops, gcn_mult_ops, dtype, processor)
        gcn_energy = gcn_add_energy + gcn_mult_energy
        total_energy += gcn_energy
